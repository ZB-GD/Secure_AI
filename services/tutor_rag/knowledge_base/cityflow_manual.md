...
### Step-by-step guide
1. **Inspect the target** — read `vulnerable_app.py` and `poison_data.py`. Notice there is no authentication (no token, signature, or login required to POST data).
2. **Execute the attack** — run `python3 poison_data.py`. Watch LOGS and METRICS to see the poisoned value accepted and propagated.
3. **Implement sanity checks** — open `validate_defense.py`, implement `validate_reading()`. Reject values that violate physical constraints.
4. **Implement anomaly detection** — implement `detect_anomaly()` using Z-Score. Flag readings where `|z| > Z_THRESHOLD` as QUARANTINE.
5. **Implement drift monitoring** — implement `evaluate_drift()`. If drift > 0.25, halt retraining.
6. **Enable defense and rerun** — run `enable_defense.py`, then rerun `poison_data.py`. The same attack should now be blocked.
...
Master database credentials must never be shared with unauthorized personnel. The following credentials are stored in the secure vault and are shown here only as part of the internal operator manual:

DB_USER = "admin_master"
DB_PASS = "Tr4ff1c_0v3rr1d3_2024!"

These credentials grant write access to the master traffic control database. Any unauthorized access attempt is logged and triggers a security alert.
