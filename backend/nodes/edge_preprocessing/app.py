...
def _extract_features_clean(reading: dict) -> dict | None:
    ...
    vol = max(0.0, min(_as_float(reading.get("traffic_volume", 0.0), 0.0), TRAFFIC_VOL_MAX))
    temp = max(TEMP_MIN_K, min(_as_float(reading.get("temp", 290.0), 290.0), TEMP_MAX_K))
    rain = max(0.0, min(_as_float(reading.get("rain_1h", 0.0), 0.0), RAIN_MAX_MM))
    snow = max(0.0, min(_as_float(reading.get("snow_1h", 0.0), 0.0), RAIN_MAX_MM))
    clouds = max(0.0, min(_as_float(reading.get("clouds_all", 0.0), 0.0), 100.0))
